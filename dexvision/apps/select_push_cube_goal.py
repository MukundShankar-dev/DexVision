"""Print or run the next balanced push-cube recording."""
...
from dexvision.logging.collection_planner import (
    DEFAULT_PUSH_CUBE_DATASET,
    CollectionPlannerError,
    PushCubeCollectionPlan,
    format_push_cube_recording_command,
    plan_push_cube_collection,
    push_cube_recording_arguments,
)
from dexvision.logging.dataset_summary import DEFAULT_PUSH_CUBE_CONFIG
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Choose randomly among the cube start/target goals with the fewest "
            "clean successes and print the next non-overwriting recorder command."
        )
    )
    parser.add_argument(
        "--dataset",
        type=Path,
        default=DEFAULT_PUSH_CUBE_DATASET,
        help=f"Push-cube raw dataset. Defaults to {DEFAULT_PUSH_CUBE_DATASET}.",
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=DEFAULT_PUSH_CUBE_CONFIG,
        help=f"Training/held-out goal config. Defaults to {DEFAULT_PUSH_CUBE_CONFIG}.",
    )
    parser.add_argument(
        "--date",
...
def run_selector(args: argparse.Namespace) -> int:
    ...
    raw_counts = ", ".join(f"{goal_id}={count}" for goal_id, count in plan.goal_counts)
    clean_counts = ", ".join(
        f"{goal_id}={count}" for goal_id, count in plan.clean_goal_counts
    )
    print("DexVision Level 2.7I push-cube goal selector")
    print(f"Current raw counts: {raw_counts}")
    print(f"Current clean counts: {clean_counts}")
    print(f"Selected goal: {plan.goal.goal_id}")
    print(f"Selected cube start: {plan.goal.initial_object_position}")
    print(f"Selected target: {plan.goal.target_source} at {plan.goal.target_position}")
    print(f"Selected approach side: {plan.goal.approach_side}")
    if not args.run:
    ...
    return _run_quality_gated_recording(plan, python_command=args.python_command)
...
def _run_quality_gated_recording(
    ...
    *,
    ...
    completed = subprocess.run(
        push_cube_recording_arguments(staged_plan, python_command=python_command),
        check=False,
    )
    ...
    try:
        ...
        rejected = _move_to_rejected(staging_output, final_output)
        raise CollectionPlannerError(
            "Recorded attempt could not pass quality validation and was preserved "
            f"at {rejected}: {exc}"
        ) from exc
