"""Print or run the next balanced button-press recording."""
...
from dexvision.logging.collection_planner import (
    DEFAULT_BUTTON_PRESS_DATASET,
    ButtonPressCollectionPlan,
    CollectionPlannerError,
    button_recording_arguments,
    format_button_recording_command,
    plan_button_press_collection,
)
from dexvision.logging.dataset_summary import DEFAULT_BUTTON_PRESS_CONFIG
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Choose randomly among the button/depth goals with the fewest clean "
    ...
    parser.add_argument(
        "--dataset",
        type=Path,
        default=DEFAULT_BUTTON_PRESS_DATASET,
        help=f"Button-press raw dataset. Defaults to {DEFAULT_BUTTON_PRESS_DATASET}.",
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=DEFAULT_BUTTON_PRESS_CONFIG,
        help=f"Training/held-out goal config. Defaults to {DEFAULT_BUTTON_PRESS_CONFIG}.",
    )
    parser.add_argument(
        "--date",
...
def run_selector(args: argparse.Namespace) -> int:
    ...
    print("DexVision Level 2.7H button-press goal selector")
    print(f"Current raw counts: {raw_counts}")
    print(f"Current clean counts: {clean_counts}")
    print(f"Selected goal: {plan.goal.goal_id}")
    print(f"Selected button: {plan.goal.button_id}")
    print(f"Selected press depth: {plan.goal.target_press_depth:.3f} m")
    if not args.run:
...
def _run_quality_gated_recording(
    ...
    *,
    ...
    completed = subprocess.run(
        button_recording_arguments(staged_plan, python_command=python_command),
        check=False,
    )
